...
| A16 | v1.16: band sesi di-rework ke **Opsi A** (mockup disetujui user via preview): 1 rectangle per sesi utk HARI INI saja (bukan 4 hari); fill solid gelap + border `HUNT_COL_EDGE_*` tipis (live: width 2 + fill varian terang); label 2-baris `NAMA jj:00-jj:00 / H <x> L <y> | Range|Breakout Up/Down|Invalidated` (sumber `SOpenRange.status` — nol logika baru); signature `RenderSessionBands(sessions,data,nowBrk)`; ledger/input tetap | ≤3 band+3 label vs ≤12; chart lebih senyap & ringan | riwayat sesi hari lampau tidak lagi tergambar |
| A17 | v1.17: **background warna DIHAPUS** (feedback user) — RenderSessionBands kini Opsi A final: outline rectangle TANPA fill (FILL=false, tepi `HUNT_COL_EDGE_*` tipis; live: width 2) dari jam awal→akhir sesi, batas harga = **high/low sesi hari ini** dihitung dari cache `CDataService.GetClosedBar` (scan mundur, break saat keluar jendela; ≤600 bar; nol CopyRates tambahan) — sesi live: edge kanan = bar closed terakhir + PeriodSeconds (update per bar, non-repaint intra-bar). Label 1-baris `NAMA H x L y | status(OR)` di atas kotak; tooltip = jam broker + pips. Define warna band (6×) dihapus; purge OnInit membersihkan versi lama | kotak kontekstual yang menempel ke harga, bukan pita latar | saat sesi belum ada bar closed, kotak tak digambar (by design) |
| A18 | v1.18: (a) kotak sesi dapat **fill LATAR di DALAM kotak** (rect ke-2 `HUNT_SESS_<s>bg`, `HUNT_COL_BOX_FILL_*` solid gelap + varian live, FILL=true BACK=true ZORDER −1, ledger sama) — outline v1.17 tetap; (b) news: `SNewsStatus.err` menampilkan PENYEBAB di baris dashboard (`NO DATA — WebRequest err=4060 (cek Allow WebRequest URL)` / `STALE — …`), OnInit cetak panduan whitelist URL + interval retry saat fetch pertama gagal — endpoint divalidasi ulang 2026-09-02 (feed sslecal2 hidup, importance=3 server-side) | fill terkurung H/L kotak → tidak menutupi candle di luar sesi; diagnostik news tanpa buka log | tester: news tetap tanpa data (WebRequest tak tersedia) — normal |
| A19 | v1.19: `InpForceGridOff` (default true) — OnInit `ChartSetInteger(0,CHART_SHOW_GRID,false)`; EA sebelumnya TIDAK PERNAH menyentuh properti chart (dibuktikan grep) — grid 'nyala lagi' berasal dari template Default pada TAB BARU tiap siklus update; now tiap attach = grid off, konsisten juga di visual tester. Set false = serahkan ke template user | satu sumber kebenaran tampilan; tanpa ubah template global | user yang SUKA grid matikan inputnya, jangan hapus dari template (konflik) |
